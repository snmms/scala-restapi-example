...
class ProductoRepositoryImpl @Inject()(
  ...
  override def findAll(categoria: Option[Long], precioMin: Option[Double], precioMax: Option[Double],
    ...
    Iterator.continually(rs).takeWhile(_.next()).map(rsToProducto).toSeq
  }
  ...
  override def findById(id: Long): Future[Option[Producto]] = Future {
    ...
    stmt.setLong(1, id)
    val rs = stmt.executeQuery()
    if (rs.next()) Some(rsToProducto(rs)) else None
  }
  ...
  override def update(id: Long, nombre: Option[String], descripcion: Option[String],
    ...
    findById(id).flatMap {
      case Some(p) =>
        ...
        val newDesc: Option[String] = descripcion.orElse(p.descripcion)
        ...
        Future {
          ...
          stmt.setLong(7, id)
          stmt.executeUpdate()
        }.map(_ => Some(p.copy(nombre = newNombre, descripcion = newDesc, precio = newPrecio,
